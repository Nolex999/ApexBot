...
def add_log(msg, level="info"):
    extended_state["activity_log"].insert(
        0, {"time": datetime.now().isoformat(), "msg": msg, "level": level}
    )
...
def register_routes(app, bot_state):
    ...
    def save_keys():
        ...
        # Reinit exchange in data_handler if available (en gardant le weight tracker)
        ...
        if dh:
            ...
            # Ré-installe le tracker de weight Binance sur le nouvel exchange
            try:
                dh._install_weight_tracker()
            except Exception:
                pass
    ...
    def toggle_auto():
        if not _auth():
            return jsonify({"error": "Unauthorized"}), 401
        extended_state["bot_control"]["auto_trade"] = not extended_state["bot_control"][
            "auto_trade"
        ]
        state = extended_state["bot_control"]["auto_trade"]
        add_log(
            f"{'🤖' if state else '🖐️'} Auto-trade {'activé' if state else 'désactivé'}",
            "info",
        )
    ...
    def get_alerts():
        ...
        return jsonify(
            {
                "alerts": extended_state["alerts"],
                "triggered": extended_state["triggered_alerts"][-20:],
            }
        )
    ...
    def add_alert():
        ...
        add_log(
            f"🔔 Alerte créée: {alert['symbol']} {alert['condition']} {alert['price']}",
            "info",
        )
    ...
    def delete_alert(alert_id):
        if not _auth():
            return jsonify({"error": "Unauthorized"}), 401
        extended_state["alerts"] = [
            a for a in extended_state["alerts"] if a["id"] != alert_id
        ]
    ...
    def delete_note(note_id):
        if not _auth():
            return jsonify({"error": "Unauthorized"}), 401
        extended_state["notes"] = [
            n for n in extended_state["notes"] if n["id"] != note_id
        ]
        return jsonify({"ok": True})
        ...
        return jsonify(
            {
                ...
                "keys_configured": extended_state["api_keys"]["configured"]
                ...
                "alerts_count": len(
                    [a for a in extended_state["alerts"] if a["active"]]
                ),
